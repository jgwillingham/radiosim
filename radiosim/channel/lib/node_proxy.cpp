...
// Constructor
NodeProxy::NodeProxy(zmq::context_t* p_ctx, short txport, short rxport, int buffer_size){
	init_sockets(p_ctx, txport, rxport);
	init_buffers(buffer_size);
}


// Initialize sockets
void NodeProxy::init_sockets(zmq::context_t* p_ctx, short txport, short rxport){
	this->txport = txport;
	this->rxport = rxport;

	txsocket = zmq::socket_t(*p_ctx, zmq::socket_type::pull);
	txsocket.bind( "tcp://127.0.0.1:" + std::to_string(txport) );

	rxsocket = zmq::socket_t(*p_ctx, zmq::socket_type::push);
...
void NodeProxy::txlisten(){
	...
	while (true) {
		...
		std::cout << "Received data " << msg.to_string() << std::endl;
	}
}
